...
Phase 1〜3の採用管理・Discovery・AI Scoutに加え、Phase 3.5のデータ品質基盤とPhase 4のVisual Reverse Searchを実装しています。
...
- 権利確認付き参考CGアップロード、視覚特徴分析、Visual Fit Scoreと差異説明
- 非公開Storage、30日保持、即時削除、期限切れCron、利用量・監査記録
...
- `visual-search-quarantine`、`visual-search-references`の非公開Storageと所有者RLS
...
同じ`CRON_SECRET`で20:30 UTCに`/api/cron/visual-search-cleanup`を実行し、30日を過ぎた参考画像と派生データをStorage API経由で削除します。
...
`Visual Search`は権利確認済みの参考CGを1〜5枚受け付けます。画像は8MB以下のJPEG/PNG/WebPに限定し、magic bytesとデコードを検証してWebPへ再エンコードします。非公開Storageに30日保存し、手動で即時削除できます。全候補画像を毎回送らず、DB事前順位の上位20名について既存の画像ベース8軸評価と公開職務情報をAIで再評価し、最大10名を表示します。
