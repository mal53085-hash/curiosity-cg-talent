...
v0.5.0では日本国内企業の実採用を主導線にし、Home、Candidates、Add Candidates、Hiring Pipelineの4画面で日常業務を完結できます。従来のAI Scout、Visual Search、Discovery、評価・検証機能は削除せずAdvancedに維持しています。
...
- Simple（既定）/ Advancedをユーザー単位で切り替える日本採用ワークスペース
- 今日の新着、勤務条件未確認、返信待ち、面談予定、長期未更新を示すアクション型Home
- CG Fit / Japan Readiness / Contact Priorityに絞った候補者一覧
- 日本在住、東京勤務、日本移住、海外リモート、条件未確認、今週連絡の保存済みビュー
- New / Shortlist / Contacted / Interview / Offer / ClosedのHiring Pipeline
- 各日本勤務条件の事実値と確認元を分離したJapan Readiness
- 未確認事項を質問として扱う日英スカウト文面のコピー
...
- `style_profiles`、追記型`style_profile_versions`、Visual Search / AI Scoutとの関連列と所有者RLS
...
- Japan Readiness列、後方互換Hiring Pipeline、`user_preferences`、`candidate_interactions`
...
検索中は画像検証から結果作成まで6段階を表示し、完了後は`/visual-search/[searchId]`へ移動します。結果ページはReference Analysis、派生指標の算出根拠、スタイル要約、Visual / AI Overall / Scoutの独立スコア、共通点・差異・面談質問を表示します。保存済み検索は開く、特徴量だけの再検索、複製、名前変更、確認付き削除に対応します。

管理者が「Style Profileとして保存」を明示的に押した場合だけ、派生特徴・16次元vector・評価weights・model versionをversion 1として保存します。元画像は保存・再送信しません。ActiveなStyle ProfileはVisual Searchの再ランキングとAI Scoutの自然言語条件で選択でき、候補者詳細には過去に実行したProfile別適合点を表示します。
...
- ActiveなStyle Profileを任意選択し、自然言語条件と保存済み派生特徴量を組み合わせられます。OpenAIへ元画像は送信せず、Responses APIは`store:false`です。
