...
const schema = z.object({
  ...
  status: z.enum(["not_run", "passed", "failed", "recheck"]),
  actual_result: z.string().max(8000),
  evidence_note: z.string().max(5000),
}).superRefine((value, context) => {
  if (value.status !== "not_run" && !value.actual_result.trim()) {
    context.addIssue({ code: "custom", path: ["actual_result"], message: "実結果を入力してください。" });
  }
});
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase.from("validation_checklist_runs").insert({ checklist_id: checklist.id, status: parsed.data.status, actual_result: parsed.data.actual_result.trim() || null, evidence_note: parsed.data.evidence_note.trim() || null, verified_by: auth.user.id }).select("id,verified_at").single(); if (error) return Response.json({ error: "検証結果を保存できませんでした。" }, { status: 500 });
